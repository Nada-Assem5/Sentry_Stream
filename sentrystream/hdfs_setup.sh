...
MAX_WAIT=300
WAITED=0
until docker exec sentry-hive-server bash -c "echo > /dev/tcp/localhost/10000" 2>/dev/null; do
...
docker ps --filter "name=sentry-"

echo "Step 3: Creating database and tables in Hive..."
docker cp ./sql/setup.sql sentry-hive-server:/tmp/setup.sql
docker exec -it sentry-hive-server beeline \
...
echo "Load it manually: docker cp <path>\\dataset.csv sentry-hive-server:/tmp/dataset.csv"
